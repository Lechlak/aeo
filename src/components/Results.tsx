...
export const Results: React.FC<ResultsProps> = ({ analysis }) => {
  ...
  return (
    <div id="results-content" className="mt-8 bg-white shadow rounded-lg overflow-hidden transition-all duration-500 ease-in-out">
      <div className="p-6 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Analysis Results</h2>
            <p className="mt-1 text-sm text-gray-500">{formatDate(analysis.timestamp)}</p>
            <p className="text-base font-medium text-gray-800 mt-2 truncate max-w-md">{analysis.url}</p>
          </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="flex -mb-px">
          {['overview', 'seo', 'aeo', 'recommendations'].map(tab => (
            <button
              key={tab}
              ...
              className={`w-1/4 py-4 px-1 text-center border-b-2 font-medium text-sm ${
                activeTab === tab
                  ? {
                      overview: 'border-blue-500 text-blue-600',
                      seo: 'border-blue-500 text-blue-600',
                      aeo: 'border-purple-500 text-purple-600',
                      recommendations: 'border-green-500 text-green-600',
                    }[tab]
